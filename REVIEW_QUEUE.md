# Priority Review Queue

Use this board to track priority public-facing surfaces and keep publishing predictable.

This is not a complete website inventory. It records front doors, active review priorities, and surfaces that need publication/boundary decisions. Folder indexes remain the complete navigation source for each lane.

This queue records website visibility only. The private source/member container is tracked in `Components/the-vessel/QUEUE.md`.

For the operational meaning of `public`, `community`, and `private`, see
`ACCESS_BOUNDARIES.md`. Frontmatter is not access control.

---

## Public Front Doors And Holds

- `index.html` — primary visual site front door. Mother Spirit and The Water of Life remain open; Sovereign Biophysics now opens through its book index, with direct homepage paths to **Golden Lexicon** and **How to Read This Archive**. Foundations, Relationship, and The Vessel remain held without links.
- `README.md` — repository-facing site index.
- `sovereign-biophysics-distillation/index.html` — public book index with the approved gold-and-silver emblem; **Golden Lexicon**, **Manifesto of the Clear Mirror**, **The First Water**, **Women of Alchemy**, **The Jar Held to the Light**, **Shivambu**, and **How to Read This Archive** are open, while all other chapter and archive-room links remain held closed.
- `sovereign-biophysics-distillation/README.md` — repository-facing Sovereign Biophysics index.
- `sovereign-biophysics-distillation/00-orientation/golden-lexicon.html` — public designed reading edition of the Golden Lexicon; linked from the Sovereign Biophysics index after visual, source, safety, and navigation review on 2026-09-05.
- `sovereign-biophysics-distillation/00-orientation/manifesto-clear-mirror.html` — public designed reading edition of Chapter II; approved and linked from the Sovereign Biophysics index on 2026-09-06.
- `sovereign-biophysics-distillation/00-orientation/the-first-water.html` — public designed reading edition of Chapter III; approved and linked from the Sovereign Biophysics index on 2026-09-06.
- `sovereign-biophysics-distillation/00-orientation/women-of-alchemy.html` — public designed reading edition of Chapter IV; approved and linked from the Sovereign Biophysics index on 2026-09-06.
- `sovereign-biophysics-distillation/00-orientation/the-jar-held-to-the-light.html` — public designed reading edition of Chapter V; Ana confirmed permission for Olivier's name in this context, approved the chapter, and opened it from the Sovereign Biophysics index on 2026-09-06.
- `sovereign-biophysics-distillation/00-orientation/shivambu.html` — public designed reading edition of Chapter VI; approved by Ana and opened from the Sovereign Biophysics index on 2026-09-07. Damar Tantra verses 6–7 and 10 checked against the held translation; *pūtimuttabhesajja* located at *Mahāvagga* I.30.4. Chapter V's field plate updated to the feminine v2 artwork in the same batch.
- `The-Vessel/README.md` — displayed as **The Living Record**, temporarily unlinked from the site front door.
- `The-Vessel/portal.md` — displayed as **Private Vessel Access**, temporarily unlinked from the site front door.

All Markdown sources under `sovereign-biophysics-distillation/` are excluded from the website build by default. Only a separately reviewed, designed HTML edition may open as a reader-facing page.

---

## The Living Record

Public companion layer: dreams, body, time, backstory, and the life behind Sovereign Biophysics.

- `The-Vessel/backstory/the-record.md`
- `The-Vessel/living-year.md`
- `The-Vessel/journal/README.md`
- `The-Vessel/journal/living-journal-2026.md`
- `The-Vessel/journal/threshold-dreaming.md`
- `The-Vessel/journal/kernel-of-sight-corn-dream.md`
- `The-Vessel/journal/what-no-longer-needs-carrying.md`
- `The-Vessel/journal/the-cord.md`
- `The-Vessel/journal/the-deepest-layer.md`
- `The-Vessel/journal/the-veil-grows-thin.md`

---

## Sovereign Biophysics

### Entry And Reference

- `sovereign-biophysics-distillation/00-orientation/how-to-read-this.md` — published 2026-09-04 with a designed direct-reading edition; phrase-aware hero-deck composition approved 2026-09-06.
- `sovereign-biophysics-distillation/00-orientation/where-to-begin.md`
- `sovereign-biophysics-distillation/00-orientation/the-archive-learns-to-read-itself.md`
- `sovereign-biophysics-distillation/00-orientation/the-archive-as-biological-memory.md`
- `sovereign-biophysics-distillation/Dictionary/README.md`
- `sovereign-biophysics-distillation/10-method/plasma-forms-taxonomy.md`
- `sovereign-biophysics-distillation/10-method/sovereign-biophysics-translation-map.md`

### Core Public Orientation

- `sovereign-biophysics-distillation/00-orientation/manifesto-clear-mirror.md` — published as a designed HTML edition on 2026-09-06.
- `sovereign-biophysics-distillation/00-orientation/the-first-water.md` — published as a designed HTML edition on 2026-09-06.
- `sovereign-biophysics-distillation/00-orientation/women-of-alchemy.md` — published as a designed HTML edition on 2026-09-06.
- `sovereign-biophysics-distillation/00-orientation/the-measure-of-your-fire.md` — new draft (2026-07-28), awaiting Ana's review
- `sovereign-biophysics-distillation/00-orientation/the-grief-that-was-never-recorded.md`
- `sovereign-biophysics-distillation/00-orientation/the-language-of-erasure.md`
- `sovereign-biophysics-distillation/00-orientation/inanna-she-who-contains-all.md`
- `sovereign-biophysics-distillation/00-orientation/two-distillates.md`
- `sovereign-biophysics-distillation/00-orientation/the-distillery-in-the-field.md`
- `sovereign-biophysics-distillation/00-orientation/the-biological-baseline.md`
- `sovereign-biophysics-distillation/00-orientation/thesis-endogenous-alchemy.md`
- `sovereign-biophysics-distillation/00-orientation/a-year-in-the-practice.md`
- `sovereign-biophysics-distillation/00-orientation/a-related-body-of-work.md`

### Method, Experiments, Analysis, Synthesis

- `sovereign-biophysics-distillation/10-method/before-you-begin.md`
- `sovereign-biophysics-distillation/10-method/on-sodium-chloride.md`
- `sovereign-biophysics-distillation/10-method/ambrosia-diet.md`
- `sovereign-biophysics-distillation/10-method/the-living-loop.md`
- `sovereign-biophysics-distillation/10-method/amaroli-protocols.md`
- `sovereign-biophysics-distillation/10-method/distilled-fraction-protocols.md`
- `sovereign-biophysics-distillation/10-method/apothecary-balm.md`
- `sovereign-biophysics-distillation/10-method/oral-care-protocol.md`
- `sovereign-biophysics-distillation/10-method/observation-protocol-handout.md`
- `sovereign-biophysics-distillation/10-method/sediment-field-guide.md`
- `sovereign-biophysics-distillation/20-experiments/experiment-report-aged-plasma-distillation.md`
- `sovereign-biophysics-distillation/20-experiments/experiment-report-ana-distillation-feb22.md`
- `sovereign-biophysics-distillation/20-experiments/experiment-report-june4-fresh-plasma-h2-retention.md`
- `sovereign-biophysics-distillation/20-experiments/experiment-report-june20-aged-blend-fractionation.md`
- `sovereign-biophysics-distillation/30-analysis/analysis-water-plasma-fourth-phase.md`
- `sovereign-biophysics-distillation/30-analysis/analysis-the-fertilizer-we-flush.md`
- `sovereign-biophysics-distillation/30-analysis/analysis-solar-distillation-resource-recovery.md`
- `sovereign-biophysics-distillation/40-synthesis/synthesis-three-year-topical-observations.md`
- `sovereign-biophysics-distillation/40-synthesis/synthesis-current-state-may-2026.md`
- `sovereign-biophysics-distillation/40-synthesis/synthesis-current-state-june-2026.md`

---

## Other Website Lanes

Selected priority surfaces only; this section is not a full inventory.

**Foundations**
- `Foundations/plasma-foundation-of-a-new-world.md`
- `Foundations/sacred-ordinary-seed-of-matter.md`
- `Foundations/freedom-is-love-lineage-of-record.md`

**Relationship**
- `Relationship/Hing.md`
- `Relationship/Burdock.md`
- `Relationship/YerbaSanta.md`
- `Relationship/miswakStick.md`
- `Relationship/Bhringaraj.md`

**Presentations**
- `sovereign-biophysics-distillation/50-presentations/presentation-living-waters-v1.md`

---

## Private / Not Public Source Material

These are not website publication candidates in full form. They live in `Components/the-vessel/`.

- Golden Syphon source protocol.
- Protected May 2026 intimate-field records.
- Yoni Receiving Field drafts.
- Detailed operational/private experiment records without public summary.

Public summaries may be created later only when Ana explicitly approves the boundary.

---

## Cleanup Notes

- The old public `sovereign-biophysics-distillation/the-vessel.md` route is retired.
- Public companion material is now displayed as **The Living Record**.
- Private/source material is now referred to as **the private Vessel**.
